...
static _Dcomplex* _raw_radarfft(_Dcomplex* data, unsigned N, bool is_forward, bool keep_input) {
	...
	*b = cexp(_Cbuild(0., -2 * PI * k / N));
	...
	_Dcomplex* fft_a = (M == 8) ? radix8fft(new_a) : radix2fft(new_a, M);
	free(new_a);
	_Dcomplex* fft_b = (M == 8) ? radix8fft(new_b) : radix2fft(new_b, M);
	free(new_b);
	...
	_Dcomplex* conv_ab = (M == 8) ? radix8ifft(fft_ab) : radix2ifft(fft_ab, M);
	free(fft_ab);
